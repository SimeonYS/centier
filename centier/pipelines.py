...
class CcentierPipeline:

    # Database setup
    conn = sqlite3.connect('centier.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `centier`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM centier WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO centier (date, title, link, content)"
